...
namespace HexedProxy.Core
{
    internal class ProxyManager
    {
        ...
        {
            if (e == null || e.oRequest == null || !e.fullUrl.Contains("bhvrdbd")) return;
        ...
        private static void BeforeResponse(Session e)
        {
            ...
            switch (e.RequestMethod)
            {
                case "GET":
                    {
                        if (e.fullUrl.Contains("cdn.live.bhvrdbd"))
                        {
                        ...
                        else
                        {
                            switch (e.PathAndQuery)
                            {
                                ...
                                case "/api/v1/onboarding":
                                    {
                                        if (InternalSettings.CompleteTutorial)
                                        {
                                            e.utilDecodeResponse();

                                            var AllChallanges = JsonConvert.DeserializeObject<DBDObjects.OnboardingChallanges.ResponseRoot>(e.GetResponseBodyAsString());

                                            foreach (var Challenge in AllChallanges.progress)
                                            {
                                                foreach (var tutorial in Challenge.tutorials)
                                                {
                                                    if (!tutorial.completed) Task.Run(() => RequestSender.FinishTutorial(Challenge.step, tutorial.tutorialId));
                                                }
                                            }
                                        }
                                    }
